...
export function ConversationBar({ user, conversations, selectedId, onSelect, onCreated }: { user: User; conversations: Conversation[]; selectedId: string | null; onSelect: (id: string) => void; onCreated: (conversation: Conversation) => void }) {
  ...
  async function create(event: FormEvent) {
    ...
    if (!response.ok) { setError(data.detail || "Nem sikerült elindítani."); return; }
    onCreated(data); setUsername(""); setOpen(false);
  }
  return <div className="conversation-bar"><label>Beszélgetés<select value={selectedId || ""} onChange={e => onSelect(e.target.value)}>{conversations.map(item => <option value={item.id} key={item.id}>{item.kind === "family" ? "⌂ " : "↔ "}{item.title}</option>)}</select></label>{user.role === "parent" && <button onClick={() => setOpen(value => !value)}>＋ Másik szülő</button>}{open && <form onSubmit={create}><input value={username} onChange={e => setUsername(e.target.value)} placeholder="Szülő felhasználóneve" required /><button>Indítás</button>{error && <small>{error}</small>}</form>}</div>;
}
